...
from modules.selenium_parser import get_product_info

class Command(BaseCommand):
    help = 'Parses a product page using Selenium and saves the data to the database'

    def handle(self, *args, **options):
        base_url = "https://brain.com.ua/ukr/"
        query = "Apple iPhone 15 128GB Black"

        self.stdout.write(self.style.NOTICE(f"Starting Selenium parser for query: '{query}'..."))

        try:
            data = get_product_info(base_url, query)
        except Exception as e:
            self.stderr.write(self.style.ERROR(f"An error occurred during Selenium parsing: {e}"))
            return
